...
const StepWelcome = ({ onStart }) => (
  <div className="flex flex-col items-center text-center gap-8 py-6">
    ...
    <motion.div
      ...
      className="space-y-3"
    >
      <h1 className="text-3xl sm:text-4xl font-title font-bold text-heroBg leading-tight">
    ...
    >
      {HIGHLIGHTS.map(({ icon: Icon, text }) => (
        <div
          key={text}
          className="flex-1 flex flex-col items-center gap-2 bg-white rounded-2xl px-4 py-4 border border-gray-100 shadow-sm"
        >
          <Icon className="w-5 h-5 text-primary" />
          <span className="text-xs text-gray-600 font-medium text-center leading-snug">{text}</span>
        </div>
      ))}
    </motion.div>
    ...
    <motion.p
      ...
      className="text-xs text-gray-400 max-w-sm leading-relaxed"
    >
